...
module.exports = {
    createMother(req, res) {
        ...
        if (name == '' || occupation == '' || id_card == '' || email == '' || telephone == '' || child_name == '') {
            ...
            database.query('INSERT INTO mothers (name, occupation, id_card, email, telephone, child_name) VALUES($1,$2,$3,$4,$5,$6)'
                , [name, occupation, id_card, email, telephone, child_name], (err, results) => {
                    if (err) {
                        throw err;
                    }
                    res.status(201).json({ success: true, message: 'Success to add mather' })
                })
        }
    }
}